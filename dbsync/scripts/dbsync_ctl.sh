...
grep -B 1 -i corrupted "$LOGFILE"                    | grep 'File Name' | awk '{print $3}' | sort -u | grep '.arc' | grep -v '.arc.gz' | xargs rm
grep -B 1 "Foreign database file DBID: 0" "$LOGFILE" | grep 'File Name' | awk '{print $3}' | sort -u | grep '.arc' | grep -v '.arc.gz' | xargs rm
